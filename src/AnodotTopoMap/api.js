...
import { makeAnomaliesParams, makeAnomalyTimeSeriesParams, makeMetricTimeSeriesParams, makeMetricsPayload } from './makeParams';

const tokenKey = 'andt-token';
const currentToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJkYXRhIjoiNWNiNDZhZGJhNWE0YWE2MmZkOTY3ZGQ4OWZmMzQ0NDA4ZTFmMGQ5OWZmNGFiMTgzY2E4MjlkZjMzOWZjMTQ3ZDgwNzEyZDYwZWIiLCJpYXQiOjE1OTg4NjA5NDcsImV4cCI6MTYwMTQ1Mjk0N30.-NCqM2Eo2Gh61qCA4QXG_8Qbk7vYixBQqqHcshcPRFI';
...
export const getPropertiesDict = (metric) => {
    ...
    return makeRequest(url, payload)
        .then(({properties, propertyValues}) => ({
...
export const getMetricsOptions = () => {
    ...
    return makeRequest(url, payload)
        .then(data => data && data.propertyValues ? data.propertyValues.map(el => el.value) : []);
};
...
export const loadEventsData = ( { timeInterval, filters, metric }) => {
    ...
    return makeRequest(url, payload)
        .then((data = {}) => data.events?.map(e => ({...e, startDate: e.date, isEvent: true, measure: metric})))
...
export const loadMetricsTimeSeries = (metricParams, timeParams) => {
    ...
    return makeRequest(...makeMetricTimeSeriesParams(metricParams, timeParams, url))
        .then((data = {}) => {
            const meta = {
...
export const loadAnomaliesTimeSeries = (anomaly) => {
    ...
    return makeRequest(makeAnomalyTimeSeriesParams(anomaly, url))
        .then((data = {}) => data)
};
